...
"%PYTHON%" -m PyInstaller --noconfirm --clean --onedir --windowed --name TikTokAffiliateReport --specpath build --distpath "%APP_STAGE%" --workpath build\pyinstaller-work --icon "%CD%\packaging\app.ico" --add-data "%CD%\packaging\app.ico;packaging" --add-data "%CD%\web\out;web" --add-data "%CD%\tiktok_affiliate_report\migrations.py;tiktok_affiliate_report" --collect-all openpyxl --collect-submodules uvicorn --copy-metadata uvicorn --hidden-import h11 --hidden-import python_multipart --hidden-import pystray._win32 --hidden-import sqlalchemy.dialects.sqlite --hidden-import sqlalchemy.dialects.postgresql.psycopg --hidden-import tiktok_affiliate_report.auth --hidden-import tiktok_affiliate_report.api --hidden-import tiktok_affiliate_report.db --hidden-import tiktok_affiliate_report.migrations --hidden-import segno --hidden-import tiktok_affiliate_report.pairing --hidden-import tiktok_affiliate_report.parser --hidden-import tiktok_affiliate_report.reports --hidden-import tiktok_affiliate_report.updater --hidden-import tiktok_affiliate_report.version desktop_launcher.py
if errorlevel 1 goto :error
