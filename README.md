...
Tải installer từ [public GitHub Releases](https://github.com/anhtahaylove/affiliate-report/releases), đối chiếu `SHA256SUMS.txt`, rồi chạy `AffiliateReportSetup-v2.1.1.exe`.
...
2. Mở PowerShell tại thư mục tải xuống và kiểm tra hash nếu cần:
   ...
   ```powershell
   Get-FileHash .\AffiliateReportSetup-v2.1.1.exe -Algorithm SHA256
   ```
...
Tải `AffiliateReport-v2.1.1-arm64.apk` từ cùng trang Releases và đối chiếu dòng tương ứng trong `SHA256SUMS.txt`. APK hỗ trợ thiết bị Android 7.0 trở lên dùng ABI `arm64-v8a`.
...
- `artifacts\installer\AffiliateReportSetup-v2.1.1.exe`
- `artifacts\android\AffiliateReport-v2.1.1-arm64.apk`
