...
import { ApiError, CurrentUser, dailyReportExportUrl, loadCurrentUser, loadMeta, loadUiPreferences, saveUiPreferences, type UiPreferences } from "@/lib/api";
...
const routeMeta: Record<RouteKind, { label: string; title: string; copy: string; needsWrite?: boolean; needsOwner?: boolean; filters?: boolean; search?: boolean }> = {
  ...
  preferences: { label: "Giao diện", title: "Giao diện ứng dụng", copy: "Cá nhân hóa chế độ màu và cách hiển thị Momentum Canvas." },
...
export function OperationsPage({ route }: { route: RouteKind }) {
  ...
  if (!preferences) return <AuthCard message="Không thể tải tùy chọn giao diện." />;
  ...
  return (
    <AppShell {...shellProps} heading={<div className="page-heading"><p className="section-label">{meta.label}</p><h1>{meta.title}</h1><p className="subtle">{meta.copy}</p></div>}>
