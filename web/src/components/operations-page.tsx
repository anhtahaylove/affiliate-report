...
import { ApiError, CurrentUser, dailyReportExportUrl, loadCurrentUser, loadMeta } from "@/lib/api";
...
export function OperationsPage({ route }: { route: RouteKind }) {
  ...
  if (meta.needsWrite && !canWrite(user)) return <AppShell user={user} apiError={apiError} appVersion={appVersion}><Notice text="Bạn không có quyền nhập dữ liệu. Hãy liên hệ chủ sở hữu để được cấp quyền." /></AppShell>;
  if (meta.needsOwner && !isOwner(user)) return <AppShell user={user} apiError={apiError} appVersion={appVersion}><Notice text="Chỉ chủ sở hữu được truy cập trang này." /></AppShell>;

  return (
    <AppShell user={user} apiError={apiError} appVersion={appVersion} heading={<div className="page-heading"><p className="section-label">{meta.label}</p><h1>{meta.title}</h1><p className="subtle">{meta.copy}</p></div>}>
