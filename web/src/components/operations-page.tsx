...
export function OperationsPage({ route }: { route: RouteKind }) {
  ...
  return (
    <AppShell {...shellProps} heading={<div className="page-heading"><h1>{pageMeta.title}</h1><p className="subtle">{pageMeta.copy}</p></div>}>
      ...
      {route === "imports" ? <ImportsPage user={user} accounts={metaData.accounts} directory={accountDirectory} maxUploadMb={metaData.max_upload_mb} inboxDir={metaData.inbox_dir ?? null} runtimePlatform={metaData.runtime_platform} /> : null}
