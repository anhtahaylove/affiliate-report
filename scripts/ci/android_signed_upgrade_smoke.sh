...
current="$GITHUB_WORKSPACE/candidate/AffiliateReport-v2.1.3-x86_64-release.apk"
target="$GITHUB_WORKSPACE/candidate/AffiliateReport-v2.1.4-x86_64-release.apk"
...
read_token
python scripts/ci/android_runtime_smoke.py --phase seed --package "$package" --expected-version 2.1.3
adb install -r "$target"
adb shell am force-stop vn.io.huuhungn.affiliatereport
adb shell am start -W -n vn.io.huuhungn.affiliatereport/.MainActivity
python scripts/ci/android_runtime_smoke.py --phase persist --package "$package" --expected-version 2.1.4
adb shell dumpsys package vn.io.huuhungn.affiliatereport | grep -q 'versionCode=2001004'
