...
def daily_report(engine: Engine, accounts=None, start=None, end=None, statuses=None) -> pd.DataFrame:
    ...
    targets = pd.read_sql(
        select(monthly_targets).where(monthly_targets.c.account == "ALL"), engine
    )
    target_map = {
...
def monthly_kpi(engine: Engine, accounts=None, start=None, end=None, statuses=None) -> pd.DataFrame:
    targets = pd.read_sql(
        select(monthly_targets).where(monthly_targets.c.account == "ALL").order_by(monthly_targets.c.month),
        engine,
    )[["month", "target_commission"]].rename(columns={"target_commission": "daily_target"})
    df = _apply_filters(_current_rows(engine), accounts, start, end, statuses)
    if df.empty:
        actual = pd.DataFrame(columns=["month", "actual_commission", "order_lines"])
    else:
        df["order_date"] = pd.to_datetime(df["order_date"], errors="coerce")
        df = df[df["order_date"].notna()].copy()
        ...
        actual = df.groupby("month", as_index=False).agg(
            actual_commission=("actual_commission", "sum"),
            order_lines=("id", "count"),
        )
    out = targets.merge(actual, on="month", how="outer")
    if out.empty:
        return pd.DataFrame(columns=["month", "daily_target", "days_in_scope", "monthly_target", "actual_commission", "gap", "target_achievement", "order_lines"])
